...
> ⚠️ **部分过时（2026-06-15 / `3.5.28-doge.14-beta.3`）**：**故障转移功能已整体物理删除**。本文「故障转移模式」相关的全部内容（`ss_failover_enable`、`ss_status_main.sh` 常驻检测、`ss_status_reset.sh`、状态历史缓存/切换判定、daemon 模式）**已不存在**。当前状态检测**只剩轻量 serve 路径**：常驻 `status-tool serve` + 前端按需 `statusctl probe-once` 触发单次探测（即下文"非故障转移 + serve"那条，现为唯一路径）。下文"故障转移模式"各节仅作历史参考。
