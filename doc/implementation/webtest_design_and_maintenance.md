...
3. 只有后端返回 `ok1`、`ok4` 或 `ok5` 时，前端才认为当前处于真实批量测速中（`ok5` 是缓存重建阶段，仍属于本轮测速的前置步骤），并进入 websocket / 轮询跟随流程。
...
- `ok5`
  - 后端检测到节点配置缓存（`webtest cache`）缺失或过期，正在重建缓存阶段。
  - 缓存重建可能耗时数十秒到几分钟，期间任何 `latency_test('2')` 调用都会持续返回 `ok5`。
  - 前端把它当成"批量测速中"，但额外把状态条置为 `【节点配置缓存重建中...】`，并通过 `cache_rebuild_notice_shown` 标记位**对同一轮缓存重建只弹一次** `layer.msg`：
    - 收到 `ok5` 时，仅在标记为 `false` 时弹窗，弹完置 `true`。
    - 收到 `ok1 / ok4` 时（说明本轮已离开缓存重建阶段，进入真实测速），重置标记为 `false`，使下一轮真实需要重建时还能再弹一次。
    - `finish_latency_batch()` 完成后也会重置，下一轮重建可重新弹窗。
  - 这避免了页面切 tab、alt-tab 切回等自动 re-fire 路径反复触发同一条提示的"骚扰"现象。
- `ok2`
...
- 现在前端已经修正，只把 `ok1 / ok4 / ok5` 视为真实批量测速；其中 `ok5` 走缓存重建提示分支，且通过 `cache_rebuild_notice_shown` 去重。
